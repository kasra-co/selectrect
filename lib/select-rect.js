...
var SelectRect = module.exports = React.createClass({
	...
	dragHandle: function( event ) {
		...
		var newRect = geometry.constrainScale([[
			Math.min( this.state.origin[0], H1[0] ),
			Math.min( this.state.origin[1], H1[1] )
		], [
			Math.max( this.state.origin[0], H1[0] ),
			Math.max( this.state.origin[1], H1[1] )
		]], [
			bounds.width,
			bounds.height
		]);
	...
	pan: function( event ) {
		...
		newRect = geometry.constrainTranslation( newRect, bounds );
