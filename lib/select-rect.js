...
var SelectRect = module.exports = React.createClass({
	...
	dragCorner: function( event ) {
		...
		var xMaxLim = magnitude * Math.abs(( img.width - this.state.x0 ) / ( Math.cos( theta ) * magnitude ));
		var yMaxLim = magnitude * Math.abs(( img.height - this.state.y0 ) / ( Math.sin( theta ) * magnitude ));

		magnitude = Math.min( magnitude, xMaxLim, yMaxLim );
